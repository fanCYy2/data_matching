# -*- coding: utf-8 -*-
"""AUS 层3 v2(abstract-vs-abstract)验证 + 阈值标定。

真值 = 层2 机构确认(round2_inst)的 rid:host 机构过滤在同名候选里【唯一】选出一个
authorid,该 authorid 视作银真值。机构信号与语义(领域)正交,故可当独立真值来标语义阈值
——与 US(eval_sem_us.py)同口径,AUS 无 ORCID,机构是唯一免费真值。

在这些 rid 的【全部层1 同名候选】上打 v2 分,评估:
  1) acc@1:机构确认的真人能否在 v2 语义分里排第1(loose / strict / ties 并列分不开);
  2) 正确 / 错误候选的分数分布(再按作者有摘要文本 vs 回退子学科名拆分);
  3) THRESH / MARGIN 网格 → 覆盖 vs 正确 权衡,据此定 AUS_SEM_THRESH_V2 / AUS_SEM_MARGIN_V2。

前置:author_vectors_aus.npz 需覆盖这些 rid 的候选(已含 round2_inst 候选)。
运行:python eval_sem_aus.py
"""
import sys
...
import aus_match as m   # 导入即建 con / aus 视图 / host 桥;_semantic_score_v2 直接可用
from aus_embed import load_author_vectors
...
if not m._use_v2():
    raise SystemExit("[error] author_vectors_aus.npz 不存在或 AUS_SEM_V2=0;先建作者向量再标定。")

# ---- 真值:round2_inst(match_2_aus.csv 里该 rid 唯一 authorid)----
m2 = pd.read_csv("match_2_aus.csv", low_memory=False)
...
m1 = pd.read_csv("match_1_aus.csv", low_memory=False)
cin = (m1[m1["rid"].isin(truth_rids)][["rid", "aus_name", "authorid", "match_type"]]
       .drop_duplicates().copy())
cin["host_id"] = pd.NA
m.con.register("r3in", cin)
long_df = m.match_3()          # 补 aus_text / field_name / share / rnk

# ---- v2 打分(与 aus_match.py 同一实现)----
