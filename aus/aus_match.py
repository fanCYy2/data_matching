# -*- coding: utf-8 -*-
"""
AUS 澳大利亚研究理事会(ARC)资助 lead-investigator -> SciSciNet/OpenAlex authorid。

复用 EU 版 data_match.py 的四层漏斗(与 us_match.py 同构):
  层1 名字(exact / alias / fuzzy)
  层2 机构(需要 aus_host_ids_bridge.csv)
  层3 语义领域(见下)
  层4 兜底(passed -> 论文数 -> authorid)

AUS 数据与 EU/US 数据的栏目映射:
  EU Researcher(s) / US PrincipalInvestigator -> AUS lead-investigator
  EU Host / US Organization                   -> AUS current-admin-organisation
  EU Panel / US Title                          -> AUS primary-field-of-research
  US Abstract                                  -> AUS grant-summary

【AUS 特有:姓名带学术头衔】lead-investigator 每行都带头衔前缀(Prof / A/Prof / Dr /
  Em/Prof / Adj/Prof / Hon / Asst ...),而 sciscinet display_name 不带。若不去衔,
  norm("Prof Belinda Liddell") 永远配不上 "Belinda Liddell"。故先用 striptitle 宏剥掉
  开头一串头衔单元(斜杠复合如 a/prof 也识别),得到净名 inv_name,全流程用它当"研究者名"。
  只对 AUS 侧去衔(sciscinet 名本就无衔),避免误伤。实测 2220 行全部剥净、无残留、无清空。

【AUS 特有:领域自带 ANZSRC 编码前缀】primary-field-of-research 形如
  "5205 - Social and Personality Psychology",100% 带 "NNNN - " 前缀;剥掉即得干净的
  细分学科标签(和 EU 的 Panel 去 "PE9 - " 前缀同构)。这个标签本身就是很好的领域文本,
  层3 v1 直接可用。

层3 语义领域(与 US 同构):
  v1(回退,缺 author_vectors_aus.npz 或 AUS_SEM_V2=0 时):
     作者 top-3 level-1 子学科名  ×  本行 ANZSRC 领域标签  的最大 cosine。
     —— AUS 的领域标签干净且描述性强,v1 本身即较可靠。
  v2(默认,需 author_vectors_aus.npz):
     作者论文 title+abstract 质心  ×  该行 grant-summary 向量  的 cosine。
     作者无英文摘要 -> 回退用其 top-3 子学科名质心(同一嵌入空间,保证同 rid 可比)。
     离线产物:extract_abstracts_aus.py -> build_author_vectors_aus.py -> author_vectors_aus.npz。
  首跑(还没有 author_vectors_aus.npz)自动走 v1 生成候选池 match_3_aus.csv;据此建作者向量后,
...
from aus_embed import embed, centroid, load_author_vectors, AUTHORVEC_FILE   # 层3 v2 共享嵌入
...
# ===== 语义相似度参数 =====
# v1(回退):作者子学科名 × ANZSRC 领域标签。沿用 EU/US v1 尺度。
SEM_THRESH = float(os.environ.get("AUS_SEM_THRESH", "0.62"))
SEM_MARGIN = float(os.environ.get("AUS_SEM_MARGIN", "0.03"))

# v2(默认,abstract-vs-abstract):作者论文摘要质心 × ARC 项目 grant-summary。
# 量纲与 v1 不同 —— 两侧都是整段富文本的文档向量,cosine 分布整体偏低、更分散,阈值必须单独定。
#
# 【已标定 2026-08-28,eval_sem_aus.py,真值=round2_inst 机构确认的 42 个可评 rid(>=2 同名候选)】
#   acc@1=83%(真人被 v2 排第1,全 strict 无并列)。阈值网格结论:THRESH 在 0.30~0.55 表现完全
#   一致(几乎不 bind),分辨全靠 MARGIN —— margin=0.02 是 0-wrong 操作点(13 resolved/0 错);
#   margin=0.01 会引入错(2/16),margin>=0.03 只增 defer 不增准。故 THRESH 保 0.55 作无害下限、
#   MARGIN 定 0.02。
#   注意(标定局限):机构真值作者在层2 就定了、不进层3 候选池、故不在 npz,eval 打的是【回退子学科
#   质心】分(偏低偏挤:正确 mean 0.70)。真实层3 候选在 abstract 空间(每 rid top-1 sim p10=0.76,
#   仅 0.1% <0.55),THRESH 更不 bind。MARGIN(相对差)比绝对 THRESH 更可跨空间迁移;要更精的绝对
#   阈值需 abstract 空间真值(扩 extract 覆盖真值作者,或人工标一批 AUS 抽样)。
SEM_THRESH_V2 = float(os.environ.get("AUS_SEM_THRESH_V2", "0.55"))
SEM_MARGIN_V2 = float(os.environ.get("AUS_SEM_MARGIN_V2", "0.02"))

# v2 总开关:有 author_vectors_aus.npz 且未显式关闭时启用;否则回退 v1。
SEM_V2 = os.environ.get("AUS_SEM_V2", "1") == "1"


def _use_v2():
    return SEM_V2 and os.path.exists(AUTHORVEC_FILE)

# 论文数下限闸。EU 版用 30; 若 AUS 只想看全量, 可 AUS_PAPER_FLOOR=0。
PAPER_FLOOR = int(os.environ.get("AUS_PAPER_FLOOR", "30"))

USE_ALIAS = os.environ.get("AUS_ALIAS", "1") == "1"
USE_FUZZY = os.environ.get("AUS_FUZZY", "1") == "1"


# 名字归一化 / 去头衔宏。先建宏, 再建 aus 视图(视图里要用 striptitle 算 inv_name)。
#
# striptitle:剥掉 lead-investigator 开头的一串学术头衔单元(AUS 特有)。头衔单元含斜杠复合
# 形式(a/prof、em/prof、adj/prof ...),每个单元后必须跟空白才算头衔, 避免吃进真名
# (如 "Emma"≠"em ", "Drew"≠"dr ");'+' 支持连叠头衔(如 "Adj Prof")。只列学术头衔,
# 不含 Mr/Ms/Miss 等民用称谓(数据里没有, 且易误剪真名)。
con.sql(r"""
    CREATE MACRO striptitle(x) AS
    regexp_replace(x,
        '^(?:(?:a/prof|assoc/prof|em/prof|emer/prof|adj/prof|adj/assoc/prof|hon/prof|asst/prof|professor|prof|emeritus|adjunct|adj|assistant|asst|associate|assoc|honorary|hon|dr)\.?\s+)+',
        '', 'i')
""")

# 名字归一化宏(与 EU/US 版一致):连字符/破折号家族统一成空格 -> 去重音 -> 小写 -> 压缩空白。
...
# AUS.csv 注册成视图, 加行号 rid(从 0 开始), 并把去衔后的净名算成列 inv_name(全流程的研究者名)。
# 全部按 VARCHAR 读, 避免日期/金额列被 DuckDB 猜测。
SOURCE_CSV = os.environ.get("AUS_SOURCE_CSV", "AUS.csv")
_source_sql = SOURCE_CSV.replace("\\", "/").replace("'", "''")
con.sql(f"""
    CREATE VIEW aus AS
    SELECT (row_number() OVER ()) - 1 AS rid, *,
           striptitle("lead-investigator") AS inv_name
    FROM read_csv_auto('{_source_sql}', header = true, all_varchar = true)
""")


# 每个 AUS 行的 host 机构 -> OpenAlex 机构 id 桥表(row_id 与 rid 对齐)。
# 结构和 EU 版 host_ids_bridge.csv 一致。若尚未生成, 本脚本仍可跑, 只是层2 没有机构闸,
# 所有待定行会直通层3/层4。
BRIDGE_CSV = os.environ.get("AUS_HOST_BRIDGE", "aus_host_ids_bridge.csv")
if os.path.exists(BRIDGE_CSV):
    ...
    print("[warning] 未找到 aus_host_ids_bridge.csv: 层2 机构消歧将被跳过, "
          "所有名字未唯一命中的行直通层3/层4。先跑 resolve_host_ids_aus.py 生成它。")


# 第三轮语义匹配用:每个 AUS 行的领域文本。
# primary-field-of-research 形如 "5205 - Social and Personality Psychology",剥掉 "NNNN - "
# 前缀即得干净的 ANZSRC 细分学科标签;缺失(不该出现)-> NULL(该行无法做语义比较)。
con.sql(r"""
    CREATE VIEW aus_field_text AS
    SELECT rid,
        CASE
            WHEN "primary-field-of-research" IS NOT NULL AND trim("primary-field-of-research") <> ''
                THEN trim(regexp_replace("primary-field-of-research", '^[0-9]+\s*-\s*', ''))
            ELSE NULL
        END AS aus_text
    FROM aus
""")
...
def match_1a():
    """层1a 精确:norm(去衔 lead-investigator) = norm(sciscinet display_name)。"""
    return con.sql("""
        SELECT
            aus.rid            AS rid,
            aus.inv_name       AS aus_name,
            sci.authorid       AS authorid,
            sci.display_name   AS sci_name
        FROM aus
        JOIN '../sciscinet_authors.parquet' sci
          ON norm(aus.inv_name) = norm(sci.display_name)
    """).df()


def match_1b():
    """层1b 别名:展开 display_name_alternatives, 用 norm 或 wset 命中(去衔后的净名)。"""
    return con.sql("""
        WITH nm AS (
            SELECT rid,
                   inv_name       AS aus_name,
                   norm(inv_name) AS nn,
                   wset(inv_name) AS ws
            FROM aus
            WHERE inv_name IS NOT NULL AND inv_name <> ''
        ),
        alias AS (
        ...
        SELECT DISTINCT
            nm.rid, nm.aus_name, a.authorid, a.display_name AS sci_name
        FROM nm
        JOIN alias a
          ON nm.nn = a.nalias
          OR nm.ws = a.wsalias
...
def match_1c():
    ...
    return con.sql("""
        ...
        FROM aus
        JOIN '../sciscinet_authors.parquet' sci
          ON fl(sci.display_name) = fl(aus.inv_name)
        WHERE aus.inv_name IS NOT NULL AND aus.inv_name <> ''
          AND aus.rid NOT IN (SELECT DISTINCT rid FROM r1_ea)
    """).df()


def match_2():
    """层2 机构消歧:候选 author 必须在该行 AUS host 机构发过文。"""
    return con.sql("""
        WITH cand AS (
            SELECT
                pend.rid,
        ...
        SELECT c.rid, c.aus_name, c.authorid, c.match_type, c.host_id
        FROM cand c
        JOIN aff a
...
def match_3():
    """层3 语义消歧的取数部分:候选作者前3 level-1 子学科名 + 该行 aus_text。"""
    return con.sql("""
        WITH cand AS (
        ...
        SELECT c.rid, c.aus_name, c.authorid, c.match_type, c.host_id,
               t.aus_text, top3.field_name, top3.share, top3.rnk
        FROM r3in c
        JOIN aus_field_text t ON c.rid = t.rid
        LEFT JOIN top3 ON c.authorid = top3.authorid
    """).df()


_SCORE_KEYS = ["rid", "aus_name", "authorid", "match_type", "host_id"]


def _row_abstracts(rids):
    """取这些 rid 的"领域查询文本":优先整段 grant-summary,缺则 ANZSRC 领域标签,
    全缺 -> None。返回 {rid: text}。"""
    con.register("_score_rids", pd.DataFrame({"rid": list(rids)}))
    d = con.sql(r"""
        SELECT u.rid,
            CASE
                WHEN "grant-summary" IS NOT NULL AND trim("grant-summary") <> ''
                    THEN trim("grant-summary")
                WHEN "primary-field-of-research" IS NOT NULL
                     AND trim("primary-field-of-research") <> ''
                    THEN trim(regexp_replace("primary-field-of-research", '^[0-9]+\s*-\s*', ''))
                ELSE NULL
            END AS atext
        FROM aus u JOIN _score_rids s ON u.rid = s.rid
    """).df()
    return dict(zip(d["rid"], d["atext"]))


def semantic_score(long_df):
    """层3 打分调度:有 author_vectors_aus.npz 走 v2(abstract-vs-abstract),否则回退 v1。"""
    if _use_v2():
...
def _semantic_score_v1(long_df):
    """v1:候选作者 top-3 子学科名 × 该行 aus_text(ANZSRC 领域标签)的【最大】cosine。
    无领域/无 aus_text -> sim = NaN。"""
    keys = _SCORE_KEYS
    df = long_df.copy()
    texts = pd.unique(pd.concat(
        [df["aus_text"].dropna(), df["field_name"].dropna()], ignore_index=True))
    if len(texts) == 0:
    ...
    df["cos"] = [cos(f, e) for f, e in zip(df["field_name"], df["aus_text"])]
    return (df.groupby(keys, dropna=False)["cos"].max()
              .reset_index().rename(columns={"cos": "sim"}))


def _semantic_score_v2(long_df):
    """v2:作者论文摘要质心 × 该行 ARC grant-summary 向量 的 cosine。

    - 作者表征:在 author_vectors_aus.npz 里 -> 论文文本质心(富信号);
                否则 -> 其 top-3 子学科名质心(回退,同一嵌入空间,保证同 rid 可比)。
    - 查询表征:该 rid 的 grant-summary(缺则 ANZSRC 领域标签,见 _row_abstracts)整段直接嵌入。
    无查询文本 / 作者无任何表征 -> sim = NaN(交层4 按论文数兜底)。
    """
    keys = _SCORE_KEYS
    df = long_df.copy()

    # 每 rid 一个查询文本(整段 grant-summary),嵌入成目标文档向量
...
def match_4():
    ...
    return con.sql("""
        WITH cand AS (
            SELECT DISTINCT rid, aus_name, authorid, match_type, host_id, passed FROM r4in
        ),
        pcnt AS (
        ...
        ranked AS (
            SELECT c.rid, c.aus_name, c.authorid, c.match_type, c.host_id, c.passed,
        ...
        SELECT rid, aus_name, authorid, match_type, host_id, passed, n_papers
        FROM ranked
        WHERE rnk = 1
    """).df()
...
def homonym_reopen(uniq_rids, r1):
    ...
    与 EU/US 版同口径, 只负责补候选; 消歧仍交给层2/层3/层4。
    """
    cols = ['rid', 'aus_name', 'authorid', 'match_type']
    empty = r1.iloc[:0][['rid', 'aus_name', 'authorid']].assign(match_type='fuzzy')[cols]

    ea = r1[r1['match_type'].isin(['exact', 'alias'])][['rid', 'aus_name']].drop_duplicates()
    if ea.empty:
        return set(), empty
    ...
    cand = con.sql("""
        WITH rids AS (
            SELECT DISTINCT r.rid, r.aus_name, h.host_openalex_id AS host_id
            FROM reopen_rids r JOIN host h ON r.rid = h.row_id
            WHERE h.host_openalex_id IS NOT NULL AND h.host_openalex_id <> ''
        ),
        flh AS (
            SELECT DISTINCT r.rid, r.aus_name, sci.authorid, sci.display_name AS cname
            FROM rids r
            JOIN '../sciscinet_authors.parquet' sci
              ON fl(sci.display_name) = fl(r.aus_name)
        ...
        SELECT rid, aus_name, authorid, cname FROM flh
    """).df()
    if cand.empty:
        return set(), empty

    cand = cand[cand.apply(lambda r: names_match(r['aus_name'], r['cname']), axis=1)]
    if cand.empty:
        return set(), empty

    inject = cand[['rid', 'aus_name', 'authorid']].copy()
...
def main():
    cols = ['rid', 'aus_name', 'authorid', 'match_type', 'host_id', 'source']
    resolved = []
    ...
    l3_noinst = pend[pend['rid'].isin(noinst_rids)][['rid', 'aus_name', 'authorid', 'match_type']].copy()
    ...
    print('层3 语义打分:%s (thresh=%.2f, margin=%.2f)'
          % ('v2 作者摘要质心 × grant-summary' if _use_v2()
             else 'v1 子学科名 × ANZSRC 领域标签', sem_thresh, sem_margin))
    ...
    r1.to_csv("match_1_aus.csv", index=False, encoding='utf-8-sig')
    r2.to_csv("match_2_aus.csv", index=False, encoding='utf-8-sig')
    r3.to_csv("match_3_aus.csv", index=False, encoding='utf-8-sig')
    r4.to_csv("match_4_aus.csv", index=False, encoding='utf-8-sig')
    final.to_csv("matched_final_aus.csv", index=False, encoding='utf-8-sig')
    held.to_csv("held_lowpaper_aus.csv", index=False, encoding='utf-8-sig')

    total = con.sql("SELECT count(*) FROM aus").fetchone()[0]
    n_matched, n_held = len(final), len(held)
    print()
    print('汇总:已唯一确定的 AUS 记录(论文数下限闸 = %d)' % PAPER_FLOOR)
    ...
    print('结果已保存:match_1_aus.csv, match_2_aus.csv, match_3_aus.csv, match_4_aus.csv, '
          'matched_final_aus.csv, held_lowpaper_aus.csv')
