# -*- coding: utf-8 -*-
"""AUS 层3(v2 原生 abstract-vs-abstract)共享嵌入工具。

build_author_vectors_aus.py 与 aus_match.py 共用同一 bge 模型 / CLS 池化 / L2 归一,
保证【作者论文摘要质心】与【ARC 项目 grant-summary 查询向量】落在同一空间、可直接做 cosine。

与 US 的 us_embed.py 完全同构(只换产物文件名后缀 _aus):两侧都有富文本——ARC 项目
grant-summary vs 作者论文 title+abstract,不需要外部分类体系(ERC 面板原型)。

产物文件:
  author_vectors_aus.npz  ids(N,) + mat(N,384) + ndoc(N,)  —— AUS 候选作者论文文本质心
"""
import os
import functools
import numpy as np
...
AUTHORVEC_FILE = "author_vectors_aus.npz"
EMB_DIM = 384
...
def embed(texts, maxlen=256, bs=64):
    ...
    maxlen:富文本(grant-summary / 论文 title+abstract)用 256;短标签(子学科名)可传 64。
    """
