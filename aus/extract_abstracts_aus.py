# -*- coding: utf-8 -*-
"""
从 92GB 的 sciscinet_papertitleabstract.parquet 里,只抽出【AUS 层3/层4 候选作者】论文的
标题+摘要,产出小 parquet 供 AUS 层3(v2 abstract-vs-abstract)用。

结构与 US 版 extract_abstracts_us.py 一致,只是候选池换成 AUS 的 match_3_aus.csv、输出加 _aus 后缀。

filter-first:先把"需要的 paperid"缩到几万,再对 92GB 做【一次】半连接扫描。
- 候选作者 = match_3_aus.csv 里的 authorid(AUS 层3 候选池,已含流向层4 的)
- 每作者封顶 K 篇(默认 30,给 language='en' 过滤留余量)
- 只留 language='en'
- 摘要在库里是 OpenAlex 倒排索引(JSON:词->位置),抽出后还原成纯文本

前置:先跑一次 aus_match.py(无 author_vectors_aus.npz 时层3 自动走 v1)生成 match_3_aus.csv。

产出:
- cand_papers_aus.parquet     (authorid, paperid, rnk)   capped 映射
- cand_abstracts_aus.parquet  (paperid, title, abstract, language)  已还原摘要
"""
import os
import json
import time
...
K = int(os.environ.get("AUS_ABS_K", "30"))          # 每作者封顶论文数(pre-scan)
ABS_FILE = "../sciscinet_papertitleabstract.parquet"
CAND_CSV = os.environ.get("AUS_CAND_CSV", "match_3_aus.csv")

if not os.path.exists(CAND_CSV):
    raise SystemExit(f"[error] 找不到候选池 {CAND_CSV};请先跑一次 aus_match.py 生成它。")
...
con.sql("COPY cand_papers TO 'cand_papers_aus.parquet' (FORMAT parquet)")
...
# 4) 落盘
out.to_parquet("cand_abstracts_aus.parquet", index=False)
print(f"[4/4] 写出 cand_abstracts_aus.parquet  行 {len(out)}, 有摘要 {n_abs} "
