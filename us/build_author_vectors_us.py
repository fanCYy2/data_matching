# -*- coding: utf-8 -*-
"""离线预计算:US 候选作者(层3/层4 候选池)的"论文文本质心"向量。

输入 cand_papers_us.parquet + cand_abstracts_us.parquet(由 extract_abstracts_us.py 产出),
每个作者取 <= K 篇英文论文的 (title. abstract),一次性批量嵌入,按作者取质心。
产物 author_vectors_us.npz 供 us_match.py 层3(v2 abstract-vs-abstract)用;
...
from us_embed import embed, centroid, AUTHORVEC_FILE

K = int(os.environ.get("US_AUTHORVEC_K", "20"))   # 每作者最多用多少篇英文论文建质心
...
df = con.sql(f"""
    WITH picked AS (
        ...
        FROM 'cand_papers_us.parquet' cp
        JOIN 'cand_abstracts_us.parquet' ca ON cp.paperid = ca.paperid
    )
    SELECT p.authorid,
           coalesce(ca.title, '') || '. ' || coalesce(ca.abstract, '') AS doc
    FROM picked p
    JOIN 'cand_abstracts_us.parquet' ca ON p.paperid = ca.paperid
    WHERE p.rk <= {K}
""").df()
